...
getgenv().XSIToggles = {}
getgenv().XSISliders = {}
getgenv().XSIColorPickers = {}
getgenv().XSITextBoxes = {}
...
function lib:Window(text, preset, closebind)
	...
	Title.Position = UDim2.new(0.0339285731, 0, 0.04000000, 0)
	...
	function tabhold:Tab(text)
		...
		function tabcontent:Toggle(text,default, callback)
			...
			function methods:SetValue(bool)
				toggled = bool
				if toggled == false then
					TweenService:Create(
						Toggle,
			...
			table.insert(XSIToggles, methods)
		...
		function tabcontent:Slider(text, min, max, start, callback)
			...
			SliderValue.FocusLost:Connect(function(EP: boolean)
				SlideCircle.Position = UDim2.new((tonumber(SliderValue.Text) or 0) / max, -6, -1.30499995, 0)
				CurrentValueFrame.Size = UDim2.new((tonumber(SliderValue.Text) or 0) / max, 0, 0, 3)
				pcall(callback, tonumber(SliderValue.Text))
			end)
			...
			function methods:SetValue(value)
				CurrentValueFrame:TweenSize(UDim2.new((start or 0) / max, 0, 0, 3), "Out", "Sine", 0.1, true)
				SlideCircle:TweenPosition(UDim2.new((start or 0) / max, -6, -1.30499995, 0), "Out", "Sine", 0.1, true)
				SliderValue.Text = tostring(start and math.floor((start / max) * (max - min) + min) or 0)
			...
			table.insert(XSISliders, methods)
			
			return methods
		end
		function tabcontent:Dropdown(text, multiple: boolean, list, callback)
			local droptog = false
			local framesize = 0
			local itemcount = 0
			local multipleSelected = {}
			...
			for i, v in next, list do
				...
				Item.MouseButton1Click:Connect(
					function()
						if multiple then
							if table.find(multipleSelected, Item) then
								table.remove(multipleSelected, table.find(multipleSelected, Item))
							elseif #multipleSelected < 3 then
								table.insert(multipleSelected, Item)
							end
							local st = " "
							for idx, val in pairs(multipleSelected) do
								st = st .. "," .. val.Text
							end
							DropdownTitle.Text = text .. " - " .. st:gsub(" ,", "")
							pcall(callback, multipleSelected)
						else
							droptog = not droptog
		...
		function tabcontent:Colorpicker(text, preset: Color3, callback)
			...
			ColorSelection.Position = UDim2.new(preset and select(3, preset:ToHSV()))
			...
			HueSelection.Position = UDim2.new(0.48, 0, 1 - select(1, preset:ToHSV()))
			...
			function methods:SetValue(color)
			...
			table.insert(XSIColorPickers, methods)
		...
		function tabcontent:Textbox(text, disapper, callback)
			...
			function methods:SetValue(text)
				if #text > 0 and not disapper then
					TextBox.Text = text
					pcall(callback, TextBox.Text)
				end
			end
			...
			table.insert(XSITextBoxes, methods)
